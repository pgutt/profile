...
Patch0:     varnish4-xxxxx.patch
BuildRoot:  %{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)
BuildRequires:  ncurses-devel libxslt groff pcre-devel pkgconfig readline-devel python-docutils
...
%install
rm -rf %{buildroot}
make install DESTDIR=%{buildroot} INSTALL="install -p" pkgdatadir=%{prefix}%{_oldincludedir}/varnish
...
%{__install} -D -m 0644 etc/example.vcl %{buildroot}%{prefix}%{_sysconfdir}/varnish/default.vcl
...
%dir %{prefix}%{_oldincludedir}/varnish
%{prefix}%{_oldincludedir}/varnish/*
%{prefix}%{_libdir}/pkgconfig/varnishapi.pc
%{prefix}%{_usr}/share/aclocal/varnish.m4
%doc LICENSE
